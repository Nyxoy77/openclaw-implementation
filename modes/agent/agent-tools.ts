//File for creating tools 
import { tool, type Tool } from "ai";
import { z } from "zod"
import type { ToolExecutor } from "./tool-executor";

/**
 * Zod defines the runtime schema for each tool's input.
 *
 * Why is this needed?
 * - TypeScript interfaces/types exist only at compile time and are erased in
 *   the generated JavaScript.
 * - Tool arguments are generated dynamically by the LLM as plain JavaScript
 *   objects (e.g. { path: "src/main.ts" }).
 * - Before invoking the tool, the AI SDK uses this Zod schema to validate that
 *   the received object has the expected shape and types.
 *
 * This prevents invalid tool calls (missing fields, wrong types, malformed
 * arguments) from reaching the executor and also provides the LLM with a
 * machine-readable description of the tool's expected inputs.
 */
